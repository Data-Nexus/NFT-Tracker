...
import { constants, ERC20Contracts } from "../../src/graphprotocol-utils";
...
export function handleOSv1Sale(event: OrdersMatched): void {
  ...
  if (tx) {
    ...
    if (!saleEntity && tx.unmatchedTransferCount > 0) {
      ...
      let currencyAddress: Address = Address.fromString(
        "0xbadfeed000000000000000000000000000000000"
      ); //update to unknown ERC20
      ...
      if (
        event.transaction.value != constants.BIGINT_ZERO ||
        event.params.price == constants.BIGINT_ONE
      ) {
        currencyAddress = Address.fromString(constants.ADDRESS_ZERO);
      }
